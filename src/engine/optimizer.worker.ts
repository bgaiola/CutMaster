...
// Runs the entire optimization off the main thread.
// Communicates via postMessage for progress & result.
// ═══════════════════════════════════════════════════════════
...
// ─── Inline generateId (avoid importing helpers that may pull in DOM stuff) ──
...
// ═══════════════════════════════════════════════════════════
// MAIN OPTIMIZATION (runs entirely in worker)
// ═══════════════════════════════════════════════════════════
...
function runOptimization(
  ...
  for (const [matCode, matPieces] of byMaterial) {
    ...
    reportProgress(basePct, matCode);
    ...
    if (config.advancedMode) {
      // ═══ ADVANCED MODE ═══════════════════════════════
      const budget = Math.min(3000, Math.max(1000, expanded.length * 6));
  ...
  while (remaining.length > 0) {
    ...
    for (const th of uhs) {
      ...
      while (curY + kerf < mat.trimTop + uh && rem.length > 0) {
        ...
        if (subH <= 0) curY = curY - stripH + stripH + kerf;
